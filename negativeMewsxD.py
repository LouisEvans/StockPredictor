...
def predicted_return(md, returns):
    ...
    print("weight_exp = " + str(weight_exp(returns)) + ", spike weight * returns = " + str(
        spike_weight(returns) * returns))
...
while True:
    ...
    for epoch in range(current_epoch - 5, current_epoch):
        returns.append(mean(
            [x['epoch_return'] for x in requests.get('http://egchallenge.tech/marketdata/epoch/' + str(epoch)).json() if
             x['epoch_return'] is not None]))
    print(returns)
